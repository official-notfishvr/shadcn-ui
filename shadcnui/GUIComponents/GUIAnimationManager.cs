...
namespace shadcnui.GUIComponents
{
    public class GUIAnimationManager
    {
        ...
        public void UpdateAnimations(bool isOpen, ref float menuAlpha, ref float menuScale, ref float titleGlow,
            ref float backgroundPulse, ref int hoveredButton, float[] buttonGlowEffects, float[] inputFieldGlow,
            ref int focusedField, ref float particleTime, ref Vector2 mousePos)
        {
            if (!guiHelper.animationsEnabled) return;
        ...
        {
            if (!guiHelper.particleEffectsEnabled) return;
            ...
            for (int i = 0; i < particleCount; i++)
            {
                ...
                Color particleColor = guiHelper.customColorsEnabled ?
                    new Color(guiHelper.accentColor.r, guiHelper.accentColor.g, guiHelper.accentColor.b, alpha) :
                    new Color(0.5f, 0.8f, 1f, alpha);

                GUI.color = particleColor;
                GUI.DrawTexture(new Rect(x * guiHelper.uiScale, y * guiHelper.uiScale,
                    4 * guiHelper.uiScale, 4 * guiHelper.uiScale), particleTexture);
            }
        }
