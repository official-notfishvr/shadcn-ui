...
namespace shadcnui.GUIComponents
{
    public class GUITextAreaComponents
    {
            ...
            params GUILayoutOption[] options)
        {
            ...
            string result;
#if IL2CPP
            result = GUILayout.TextArea(text ?? (string.IsNullOrEmpty(placeholder) ? "" : placeholder), 
                textAreaStyle, (Il2CppReferenceArray<GUILayoutOption>)layoutOptions.ToArray());
#else
            result = GUILayout.TextArea(text ?? (string.IsNullOrEmpty(placeholder) ? "" : placeholder),
                textAreaStyle, layoutOptions.ToArray());
#endif
            ...
            string placeholder = "", bool disabled = false, int maxLength = -1)
        {
            ...
            string result = GUI.TextArea(scaledRect,
                text ?? (string.IsNullOrEmpty(placeholder) ? "" : placeholder), textAreaStyle);
            ...
            bool showCharCount = true, params GUILayoutOption[] options)
        {
            ...
            if (showCharCount)
            {
                ...
                Color countColor = (maxLength > 0 && (result?.Length ?? 0) > maxLength * 0.9f) ?
